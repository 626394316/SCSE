...
class DataProcess():
    ...
    def save_txt_file(self, path):
        ...
        text_splitter = RecursiveCharacterTextSplitter(chunk_size=len(documents[0].page_content), chunk_overlap=0)
        documents_txt = text_splitter.split_documents(documents)
        self.documents_txt.append(documents_txt[0])
        print("self.documents_txt",self.documents_txt)

    def build_vectory_db(self):
        # 加载文件
        for file in os.listdir(self.base_dir):
            # 构建完整的文件路径
            # file_path = os.path.join(self.base_dir, file).replace('\\', '/')
            new_path = os.path.join("../web_demo/public/static/file", file).replace('\\', '/')
            if os.path.exists("../data/common_data/" + file):
                # 复制文件
                shutil.copy("../data/common_data/" + file, new_path)
                print(f"文件已从 {'../data/common_data/' + file} 复制到 {new_path}")
            else:
                print(f"错误：源文件 {'../data/common_data/' + file} 不存在")

            if file.endswith('.md'):
                self.save_md_file(new_path)
            if file.endswith('.txt'):
                self.save_txt_file(new_path)
